...
BEGIN
	...
	declare @idusuarioSESION varchar(max); select @idusuarioSESION = idusuario from usuarios where idusuario = @idusuario
	if @idusuarioSESION is null execute sp_error 'U', 'El usuario no existe.'

	delete from sesiones where DATEDIFF(HOUR, fcreacion, getutcdate()) > 8
